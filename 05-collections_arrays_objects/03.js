...
// * 2. Log only the names of stores in state "CA".
console.log("2. Log only the names of stores in state \"CA\"");
console.log("=============================================");
stores
  .filter((element) => {
    return element[2] == "CA";
...
// * 3. Log the number of Target stores in total.
console.log("3. Log the number of Target stores in total.");
console.log("============================================");
let targetCount = stores.filter(element => {return element[1] == 'Target'}).length;
console.log(`There are ${targetCount} Target stores in total`);


// * 4. Log the number of each type of store. I.e. 5 Targets, 10 K-Marts - log
// *    these with a presentation string. I.e. "There are __ K-Marts in total."
console.log("4. Log the number of each type of store.");
console.log("========================================");
let storeCount = {};
stores.forEach( (element) => {
    if (!Object.hasOwn(storeCount,element[1])) {
        storeCount[element[1]] = 1;
    } else {
        storeCount[element[1]] += 1;
    }
})
...
for (let key in storeCount) {
    console.log(`There are ${storeCount[key]} ${key}s in total `)
}
